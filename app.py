...
# Check for Mistral API Key at startup
mistral_key = os.getenv("MISTRAL_API_KEY")

# ─── Page Config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ScribeFlow AI",
    page_icon="🎬",
    layout="wide",
...
# ─── Sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div class="hero-title" style="font-size:1.6rem">🎬 Scribe<br>Flow</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-sub">Meeting Intelligence</div>', unsafe_allow_html=True)
    st.markdown("---")

    st.markdown('<span class="badge badge-purple">Input</span>', unsafe_allow_html=True)

    # File uploader (primary input)
    uploaded_file = st.file_uploader(
        "Upload Video / Audio",
        type=["mp4", "mp3", "wav", "m4a", "webm", "ogg", "flac", "mkv", "avi"],
        help="Upload a meeting recording directly — works every time!",
    )

    st.markdown(
        '<div style="text-align:center;color:var(--text-muted);font-size:0.75rem;margin:0.4rem 0">── or ──</div>',
        unsafe_allow_html=True,
    )

    # YouTube URL (alternative input)
...
if not mistral_key:
    st.error("⚠️ **Mistral API Key is missing!** Please create a `.env` file in the project root and add `MISTRAL_API_KEY=your_key` to enable transcription analysis and chat features.")
...
if run_btn:
    ...
    if not has_upload and not has_url:
        ...
        st.error("❌ **Sarvam API Key is missing!** Hinglish transcription requires `SARVAM_API_KEY` to be set in your `.env` file.")
    elif not mistral_key:
        st.error("❌ **Mistral API Key is missing!** Please configure `MISTRAL_API_KEY` in `.env` to proceed.")
    else:
        ...
        # Vector store collection lifecycle cleanup
        import uuid
        from langchain_chroma import Chroma
        from core.vector_store import get_embeddings, CHROMA_DIR
        ...
        try:
            ...
            for chunk_file in chunks:
                try:
                    ...
                    print(f"Error removing chunk file {chunk_file}: {ex}")
